...
function update() { //runs every ten milliseconds
    ...
    if (passingperiod) { //If it's a passing period
        bottomInfo.innerHTML = "<p>Honors Chemistry Grade:" + grade + "% Time Remaining in Passing Period: " + Math.floor(time / 1000) + "s</p>";
        time -= 10; //Count down the clock
        if (isTouching(gp, document.getElementById("savage"))) { //If the player is touching Mr. Savage
            movement = false; //Disable movement, pause stop the period, and show Mr. Savage's room.
            passingperiod = false;
            document.getElementById("savageroom").style.display = "block";
            if (time < 0) { //If the player is late, Mr. Savage will deduct 1, 5, or 10%
            ...
            } else { //If the player is on-time, there is no penalty
        ...
        if (isTouching(gp, document.getElementsByClassName("slowkids")[0]) || isTouching(gp, document.getElementsByClassName("slowkids")[1])) { //If the user is in a slow-kids zone
            if (canBeSlowed) { //And they haven't already been slowed
                if (time > -5) { //And they're not late
                    //console.log("Slow zone triggered");
                    var rand = Math.floor(Math.random() * 3);
                    if (rand == 0 || rand == 1) { //Then there's a 2/3 chance they'll be slowed down
                        ...
                        setTimeout(function () {
                            canBeSlowed = true;
                        }, 4000); //Give the player four seconds to get out of the zone
                    }
                }
            }
        }
    }
    if (inclass) { //If it's an in-class break
        if (!inTutorial) { //Allow unlimited time for the player to complete the tutorial, otherwise deduct time at the normal rate.
            time -= 10;
        ...
        if (time < 1) {
            ...
            mapY = 0;
            movement = false;
            divider("<h1>Passing Period</h1>").then(function () {
                    movement = true;
                    passingperiod = true;
...
async function startGame() {
    await divider("<h1>Ms. Guillen gave youan in-class break!</h1><h2>You have 30 seconds of bonus time to prepare for passing period.</h2>", true);
    inclass = true;
    passingperiod = false;
    time = 30000;
    movement = true;
...
function leaveMulberries() {
    ...
    y -= 100;
    movement = true;
    if (inTutorial) { //If the tutorial is active, show the new goal
...
function getSavaged(penalty) {
    ...
    if (day == days) {
        ...
        if (Math.floor(Math.random() * 2) == 0) {
            ...
            divider("<h1>Ms. Guillen gave youan in-class break!</h1><h2>You have 30 seconds of bonus time to prepare for passing period.</h2>").then(function () {
                passingperiod = false;
                inclass = true;
                movement = true;
                time = 30000;
...
function test() {
    notify('<iframe width="100%" height="100%" style="position: absolute; left: 0px; top: 0px;" src="https://scanuproductions.com/rick.mp4" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>');
}
...
async function divider(content, hideMenu) {
    document.getElementById("sectionsDivider").innerHTML = content;
